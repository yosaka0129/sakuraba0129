...
class Explosion {
  constructor(position) {
    ...
    this.shouldExplodeTwice = Math.random() < 0.4; // 40%で二段爆発
  }
...
const fireworks = [];
const MAX_FIREWORKS = 20; // 打ち上げ頻度を増やす
...
function animate() {
  ...
  for (let i = fireworks.length - 1; i >= 0; i--) {
    ...
    if (fireworks[i].isDead()) {
      fireworks.splice(i, 1); // ← 正しく閉じる
    }
  }
  renderer.render(scene, camera);
}
animate(); // ← 閉じ括弧のあとに呼び出し